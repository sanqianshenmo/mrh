<template>
  <div>
    ...
    <button class="diary-write-btn" @click="submitDiray">修改日记</button>
...
export default {
  ...
  computed: {
    curDay () {
      const self = this
      if (self.diary.writeTime) {
        return self._formatTime(new Date(self.diary.writeTime))
      }
      return '?'
    },
    pics () {
  ...
  methods: {
    ...
    submitDiray () {
      ...
      let curUploadedImgs = this.uploadedImgs.map(item => {
        if (item.indexOf('w') > -1) {
          return item.slice(1)
        }
        return item
      })
      let operationImg2arr = curUploadedImgs.map(item => {
        return 'w' + item
      })
      let operationImg = operationImg2arr.join(',')
      let operationImg2 = curUploadedImgs.join(',')
      ...
      DIARY.modifyDiary(diary).then(res => {
        // console.log(res)
        if (res.data.retCode === 0) {
          MessageBox({
            title: '提交成功',
            ...
            this.$router.replace({name: 'diary.check', query: {noteId, noteIndex, date, noteType}})
          })
        } else if (res.data.retCode === 40002) {
          console.log('token失效')
        } else {
  ...
  created () {
    ...
    let id = query.id
    this.diary.id = noteId
    this.diary.noteId = noteId
    this.date = query.date
    DIARY.detail(id).then(res => {
      if (res.data.retCode === 0) {
        for (let key in self.diary) {
          if (self.diary.hasOwnProperty(key) && res.data.data.diary[key]) {
            self.diary[key] = res.data.data.diary[key]
          }
          // 手术日期
          self.date = res.data.data.diary.operationAt
          // 选择的时间
          self.pickerValue = new Date(res.data.data.diary.writeTime.replace(/-/g, '/'))
          // 上传的图片
          self.uploadedImgs = [].concat(res.data.data.diary.operationImgList)
        }
      }
    })
  }
