<template>
  <div class="input-submit home">
    ...
    <div class="input-label">
      ...
      <div class="center-banner-join" @click="openPicker2">
        <textarea rows="1" maxlength="100" placeholder="出生时辰：请精确到15~30分以内" class="join-input font-input" v-model="birthdayTime" disabled></textarea>
      </div>
      <div class="center-banner-join">
      ...
      <!--<div class="center-banner-join">
        <input type="text" placeholder="占卜反馈：请留下email接收占卜结果" oninput="if(value.length>30)value=value.slice(0,30)" class="join-input font-input input-height" v-model="email">
      </div>-->
      <div class="center-banner-join">
        <div class="bottom">
...
export default {
  ...
  methods: {
    ...
    sumbit () {
      if (!(this.name && this.birthday && this.birthdayTime && this.phone && this.bir_province && this.bir_city && this.bir_district && this.bir_province && this.bir_city && this.bir_district)) {
        console.log(this.name, this.birthday, this.birthdayTime, this.phone, this.bir_province, this.bir_city, this.bir_district, this.bir_province, this.bir_city, this.bir_district, this.email)
        ...
        if (reg2.test(this.phone) === false) {
          ...
          this.$indicator.open()
          this.birth = this.birthday + ' ' + this.birthdayTime
    ...
    sumbits () {
      CODE.sumbitInfo(this.name, this.sex, this.phone, this.birth, this.bir_province, this.bir_city, this.bir_district, this.live_province, this.live_city, this.live_district)
        .then(res => {
          if (res.data.retCode === 0) {
            this.$indicator.close()
            this.$messagebox('提交成功', '24小时内客服会添加您的微信反馈占卜结果')
              .then(action => {
                this.sex = ''
                this.name = ''
                this.phone = ''
