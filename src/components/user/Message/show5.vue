<template>
    <div>
      <div class="nomessage" v-if="tradeNone">
        <img src="./none.png" alt="">
        <div>亲，暂时还没有人给你打赏哦!</div>
      </div>
      <ul class="shop-list clearfloat"
          v-infinite-scroll="loadMore3"
          infinite-scroll-disabled="moreLoading3"
          infinite-scroll-distance="0"
          infinite-scroll-immediate-check="false">
        <div class="box" v-for="userTrade in userTrades" v-bind:key="userTrade.id">
          <div class="box-doctory">
            <div class="box-doctory-left">
              <img :src="userTrade.avatar" alt="">
            </div>
            <div class="box-doctory-right">
              <div class="box-doctory-right-top"><span>{{userTrade.userByName.substring(-1, 10)}}</span>{{userTrade.createTime}}</div>
              <div class="box-doctory-right-bottom">打赏了你 金积分{{userTrade.money}}</div>
            </div>
      ...
      <!-- 底部判断是加载图标还是提示“全部加载” -->
      <div class="more_loading" v-show="!queryLoading3">
        <div class="spinner-box">
          <mt-spinner type="fading-circle" color="#000000" :size="20" v-if="moreLoading3&&!allLoaded3"></mt-spinner>
        </div>
        <span v-if="moreLoading3&&!allLoaded3">正在加载...</span>
        <div v-if="!allLoaded3"></div>
...
export default {
  ...
  methods: {
    loadMore3 () {
      ...
      this.tradeListPage++
      USER.userTrade(this.tradeListPage, 10)
        .then(res => {
    ...
    goToNote (val) {
      router.push('/diary/person?noteId=' + val)
    }
  },
  created () {
    USER.userTrade(1, 10)
